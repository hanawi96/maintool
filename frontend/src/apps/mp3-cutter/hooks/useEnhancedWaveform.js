...
export const useEnhancedWaveform = () => {
  ...
  const generateWaveform = useCallback(async (file) => {
    console.log('🌊 [useEnhancedWaveform] Starting waveform generation...', {
      fileName: file.name,
      fileSize: file.size,
      fileType: file.type
    });
    ...
    try {
      ...
      console.log('🎯 [useEnhancedWaveform] Calling HybridWaveformGenerator...');
      const result = await HybridWaveformGenerator.generateWaveform(file);
      
      console.log('✅ [useEnhancedWaveform] Waveform generation successful:', {
      ...
      console.log('🎯 [useEnhancedWaveform] Enhanced state updated:', {
      ...
      console.error('❌ [useEnhancedWaveform] Waveform generation failed:', error);
  ...
  const reset = useCallback(() => {
    console.log('🔄 [useEnhancedWaveform] Resetting waveform state...');
    ...
    console.log('✅ [useEnhancedWaveform] Enhanced waveform state reset complete');
  }, []);
  ...
  const setStartTimeWithValidation = useCallback((time) => {
    ...
    console.log('⏮️ [useEnhancedWaveform] Setting start time:', time, '→', validTime);
    setStartTime(validTime);
  }, [endTime]);

  const setEndTimeWithValidation = useCallback((time) => {
    const validTime = Math.max(startTime + 0.1, time);
    console.log('⏭️ [useEnhancedWaveform] Setting end time:', time, '→', validTime);
